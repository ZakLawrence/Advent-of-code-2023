...
fn parse_input(input: String){
    ...
    for symbol in symbols.iter(){
        let mut cog_numbers: Vec<i32> = Vec::new();
        for part in parts.iter(){
            let neighbours = part.find_neighbours();
            if neighbours.iter().any(|&i| i == *symbol) {
                cog_numbers.push(part.value);
            }
        }
        if cog_numbers.len() > 1 {
            let mut temp_multiply = 1; 
            for number in cog_numbers{
                temp_multiply *= number; 
            }
            total += temp_multiply;
        }
    }
